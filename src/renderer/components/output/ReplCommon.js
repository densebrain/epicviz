...
const ReplCommon = {
  ...
  indent: (code, lang = "javascript") => {
    code = (code || '').replace(/([;{])(?!\n)/g, '$1\n');
    //const preferences = global.Mancy.preferences;
    const cm = CodeMirror(document.createElement('span'), {
      value: code,
      //${ReplLanguages.getLangQualifiedName(lang)}
      mode: `text/javascript`,
      tabSize: 2,//preferences.tabSize,
      indentUnit: 2//preferences.indentUnit,
    });
  ...
  highlight: (code, lang = "javascript", indent = false) => {
  ...
  sortTabCompletion: (context, completion, completeOn) => {
    const keys = _.difference(_.keys(context), ReplContext.builtIns());
    const sorted = _.sortBy(completion);
    if(/[\.\]]/.test(completeOn)) {
      // nested
      const [right, left] = _.partition(sorted, (c) => privatePattern.test(c));
      return left.concat(right);
    }

    const user = [], sys = [];
    _.each(sorted, c => {
      const container = keys.indexOf(c) == -1 ? sys : user;
      container.push(c);
    });
    // only
    const [right, left] = _.partition(sys, (c) => privatePattern.test(c));
  ...
  shouldTriggerAutoComplete: (e) => {
    ...
    const notAllowed = [123, 125, 13, 59, 61, 10, 37, 38, 39, 40, 219, 221]; // CR;=LF(direction)[]
    const shiftNotAllowed = [57, 48, 91, 93, 123, 125, 188, 190]; // (){}<>
  ...
  funType: (fun = '') => {
    ...
    const result = code.match(funPattern);
    if(result && result.length === 2) {
      //cljs sugar
      if(fun.cljs$lang$type && fun.cljs$lang$ctorStr) {
        const ctor = fun.cljs$lang$ctorStr.replace(/^.*\//,'')
...
//  isPrintableAscii: (char) => /[ -~]/.test(char),
  ...
  isCSSColor: (color) => {
    const cssValues = ['transparent', 'initial', 'inherit', 'currentColor'];
  ...
  decodeBase64: (encoded) => {
    const data = new Buffer(encoded, 'base64');
  ...
  isBMP: (buffer) => {
    ...
    const size = ((buffer[5] << 24) | (buffer[4] << 16) | (buffer[3] << 8) | buffer[2]);
  ...
  runInContext: (js, cb) => {
    const timeout = 30000// = global.Mancy.preferences;
    ...
    try {
      ...
      //const output = script.runInContext(ReplContext.getContext(), { displayErrors: true, timeout });
  ...
  bindToReplContext: (variable, value) => {
    const result = ReplContext.getContext()[variable] = value
    return result
  },
};
