...
export default class ReplOutputFunction extends React.Component {
  ...
  getType() {
    const type = ReplCommon.funType(this.props.fun);
    return ` ${type} {}`;
  }

  getAllProps() {
    const names = Object.getOwnPropertyNames(this.props.fun);
    const symbols = Object.getOwnPropertySymbols(this.props.fun);
...
//    ReplActions.bindObjectToContext(this.props.fun, ReplOutput.transformObject(this.props.fun));
  ...
  render() {
    const label = ReplCommon.highlight(this.getType());
    return (
          ...
          : <span className='repl-entry-message-output-object'>
              ...
              <span className='object-rec'>
              {
                _.map(this.getAllProps(), (key) => {
                  const value = ReplOutput.readProperty(this.props.fun, key);
                  const keyClass = Object.prototype.propertyIsEnumerable.call(this.props.fun, key) ? 'object-key' : 'object-key dull';
                  return (
