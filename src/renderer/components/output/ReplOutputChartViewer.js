...
export default class ReplOutputChartViewer extends React.Component {
  ...
  init() {
    ...
    if(this.chartViewable) {
      ...
      const keys = _.keys(this.props.chart);
  ...
  generateColumnData(source) {
    const size = _.reduce(source, (o, arr) => {
  ...
  generateFlippedData(cols) {
    const categories = _.reduce(cols, (o, col) => o.concat(col[0]), []);

    const range = _.range(cols[0].length - 1);

    const columns = _.reduce(range, (o, r) => {
      const record = _.reduce(cols, (out, col) => {
        out.push(col[r + 1] || 0);
        return out;
      }, [`${r}`]);
      o.push(record);
  ...
  onClickLineChart(e) {
    const type = this.state.spline ? 'spline' : 'line';
  ...
  onClickAreaChart(e) {
    const type = this.state.spline ? 'area-spline' : 'area';
    this.onChangeChartType(type);
  }
  ...
  renderChart() {
    const barClazz = `fa fa-bar-chart ${this.state.type === 'bar' ? 'selected' : ''}`;
    const areaClazz = `fa fa-area-chart ${this.isAreaChart() ? 'selected' : ''}`;
    const lineClazz = `fa fa-line-chart ${this.isLineChart() ? 'selected' : ''}`;
    const pieClazz = `fa fa-pie-chart ${this.state.type === 'pie' ? 'selected' : ''}`;
