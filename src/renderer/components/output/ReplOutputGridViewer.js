...
export default class ReplOutputGridViewer extends React.Component {
  ...
  transposeGridData() {
    const toTranspose = () => {
      const gridValues = this.gridData();
      const data = _.reduce(gridValues, (o, row) => {
        _.each(row, (r, pos) => {
          o[pos] || o.push([]);
          o[pos].push(r);
        });
  ...
  renderGrid() {
    ...
    const [rowHeaders, colHeaders] = this.state.transpose
      ? [this.colHeaders, this.rowHeaders] : [this.rowHeaders, this.colHeaders];

    const data = this.state.transpose ? this.transposeGridData() : this.gridData();
    const clazzMap = {
      number: 'cm-number',
      boolean: 'cm-atom',
      object: 'cm-variable',
