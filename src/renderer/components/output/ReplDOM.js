...
const ReplDOM = {
  ...
  toHTMLBody: (str) => {
    ...
    body.innerHTML = str;
    const result = _.find(body.childNodes,
  ...
  getCursorPositionRelativeTo: (dom) => {
    ...
    const treeWalker = document.createTreeWalker(
      ...
      (node) => {
        const nodeRange = document.createRange();
        nodeRange.selectNodeContents(node);
        const filter = !node.isSameNode(range.endContainer) &&
  ...
  setCursorPositionRelativeTo: (pos, dom) => {
    const treeWalker = ReplDOM.getTextNodeTreeWalker(dom);
    let idx = pos;
