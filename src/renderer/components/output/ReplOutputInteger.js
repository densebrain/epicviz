...
export default class ReplOutputInteger extends React.Component {
  constructor(props) {
    ...
    _.each(_.keys(mode), (m) => {
      const n = `on${_.capitalize(m)}Mode`;
  ...
  toIntString(n) {
    const num = this.state.type === 'signed' ? n : (n >>> 0);
  ...
  render() {
    ...
    const clazz = `mode-group ${ hide ? 'hide' : 'show'}`;
    const tips = outOfRange ? '' : 'Click to Toggle Base/Sign Converter';
    const numClazz = `cm-number ${outOfRange ? '' : 'toggle-number'}`;
    return (
